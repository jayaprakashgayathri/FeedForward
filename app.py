...
from models import db, User, Donation, DonationRequest
...
def create_app(config=None):
    ...
    # Resolve DB URL: explicit DATABASE_URL > individual POSTGRES_* vars > SQLite fallback
...
def _register_routes(app):  # noqa: C901
    ...
    def restaurant_dashboard():
        ...
        pending_requests = (
            DonationRequest.query.join(Donation)
        ...
        completed       = Donation.query.filter_by(donor_id=current_user.id,
    ...
    def accept_request(rid):
        r = DonationRequest.query.get_or_404(rid)
        if r.donation.donor_id != current_user.id:
            flash("Unauthorized.")
            return redirect(url_for("restaurant_dashboard"))
        r.status = "accepted"
